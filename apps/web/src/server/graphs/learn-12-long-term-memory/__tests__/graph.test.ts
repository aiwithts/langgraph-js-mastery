...
describe("Lesson 12: My Memory-Enhanced Assistant", () => {
	it("exports a valid module with config and createGraph", async () => {
		const mod = await import("../index");
		expect(mod.learn12LongTermMemoryModule.config.id).toBe("my-memory-enhanced-assistant");
		expect(mod.learn12LongTermMemoryModule.createGraph).toBeInstanceOf(Function);
	});

	it("creates a compilable graph (complete Lesson 12 to pass this test)", async () => {
		const mod = await import("../index");
		const graph = mod.learn12LongTermMemoryModule.createGraph();
