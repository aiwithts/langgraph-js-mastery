...
describe("Lesson 09: Swiss Army Knife Assistant", () => {
	it("exports a valid module with config and createGraph", async () => {
		const mod = await import("../index");
		expect(mod.graph09PuttingItTogether.config.id).toBe("09-putting-it-together");
		expect(mod.graph09PuttingItTogether.config.name).toBe("Swiss Army Knife Assistant");
		expect(mod.graph09PuttingItTogether.createGraph).toBeInstanceOf(Function);
	});
